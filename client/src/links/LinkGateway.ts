...
export const LinkGateway = {
  ...
  deleteLink: async (linkId: string): Promise<IServiceResponse<{}>> => {
    try {
      return await LinkGateway.deleteLinks([linkId])
    } catch (exception) {
   ...
   * - get links by id (frontend LinkGateway)
  ...
  deleteLinks: async (linkIds: string[]): Promise<IServiceResponse<{}>> => {
    try {
      ...
      const getLinksResp = await LinkGateway.getLinksById(linkIds)
      ...
      const remainingLinksResp = await LinkGateway.getLinksByAnchorIds(anchorsList)
      ...
      return await AnchorGateway.deleteAnchors(orphanAnchors)
    } catch (exception) {
